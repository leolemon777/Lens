...
- 可选生成式模型、语义聚类与批量整理；
- Developer ID 签名/公证与未安装开发证书 Mac 的分发验证；本地安装 DMG 已可生成并校验；
- 自动更新的下载、校验与安装链路：更新清单的签名校验与降级拒绝已实现并有测试，但可信发布主机、回滚路径和自动调度都还没有，因此功能保持关闭；
- Windows 适配：里程碑 0 的 `lens-format` 已写好（读写开放格式、拒绝未来 schema/路径越界/符号链接），但开发机没有 Rust 工具链，`cargo test` 尚未执行；捕获、渲染、编辑器和更新器仍需在 Windows 实机完成。
