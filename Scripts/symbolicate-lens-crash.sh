...
if [[ $# -ne 2 ]]; then
    echo "Usage: $0 /path/to/Lens.ips /path/to/Lens.dSYM" >&2
    exit 64
fi

REPORT_PATH="$1"
DSYM_PATH="$2"
DWARF_PATH="$DSYM_PATH/Contents/Resources/DWARF/Lens"
...
if [[ ! -f "$DWARF_PATH" ]]; then
    echo "Missing Lens DWARF binary: $DWARF_PATH" >&2
    exit 66
fi
...
if [[ "$BUNDLE_ID" != "app.lens.mac" || "$APP_NAME" != "Lens" ]]; then
    echo "Crash report does not belong to Lens" >&2
    exit 65
fi
...
if [[ -z "$IMAGE_OFFSETS" ]]; then
    echo "No Lens frames on the faulting thread"
    exit 0
fi
