...
public struct AutoCameraPlanner: Sendable {
    public struct Configuration: Equatable, Sendable {
        ...
        public init(
            ...
            followSamplingInterval: Double = 0.10,
            pointerSafeArea: Double = 0.68,
            pointerHysteresis: Double = 0.16,
            pointerLookAhead: Double = 0.18,
            maximumPanSpeed: Double = 0.55,
            maximumPanAcceleration: Double = 1.60,
            maximumZoomVelocity: Double = 2.20,
            maximumTransitionDuration: Double = 0.90,
    ...
    public init(camera: AutoEditPlan.Camera) {
        ...
        case .restrained:
            Configuration(
                ...
                zoomDuration: 0.60,
                followWindow: 4.20,
                followSamplingInterval: 0.13,
                pointerSafeArea: 0.74,
                pointerHysteresis: 0.20,
                pointerLookAhead: 0.15,
                maximumPanSpeed: 0.40,
                maximumPanAcceleration: 1.00,
                maximumZoomVelocity: 1.70,
        ...
        case .balanced:
            Configuration(
                ...
                zoomDuration: 0.45,
                followWindow: 3.60,
                followSamplingInterval: 0.10,
                pointerSafeArea: 0.68,
                pointerHysteresis: 0.16,
                pointerLookAhead: 0.18,
                maximumPanSpeed: 0.55,
                maximumPanAcceleration: 1.60,
                maximumZoomVelocity: 2.20,
                maximumTransitionDuration: 0.90,
        ...
        case .active:
            Configuration(
                ...
                zoomDuration: 0.36,
                followWindow: 1.80,
                followSamplingInterval: 0.08,
                pointerSafeArea: 0.56,
                pointerHysteresis: 0.08,
                pointerLookAhead: 0.22,
                maximumPanSpeed: 0.75,
                maximumPanAcceleration: 2.20,
                maximumZoomVelocity: 3.00,
                maximumTransitionDuration: 0.75,
    ...
    public func plan(
        ...
        var activeFocusCenter: LensPoint?
    ...
    private func pointerActivityBase(
        ...
        struct Activity {
            var start: Double
            var end: Double
            var startPosition: LensPoint
            var endPosition: LensPoint
            var travel: Double
        }
    ...
    private func insertingPointerFollow(
        ...
        var lastCenter: LensPoint?
        var lastVelocity = LensPoint(x: 0, y: 0)
        for pointer in smoothedPointer where pointer.time >= 0 && pointer.time <= duration {
            ...
            guard state.scale >= 1.18 else {
                lastCenter = nil
                lastVelocity = LensPoint(x: 0, y: 0)
            ...
            let requestedCenter = clampedCenter(
                LensPoint(x: requestedX, y: requestedY),
                scale: state.scale
            )
    ...
    private func clampedCenter(_ point: LensPoint, scale: Double) -> LensPoint {
