...
public struct LensSchemaVersion: Comparable, Equatable, Sendable {
    ...
    public static func < (lhs: LensSchemaVersion, rhs: LensSchemaVersion) -> Bool {
        (lhs.major, lhs.minor) < (rhs.major, rhs.minor)
    }
}

public enum LensSchemaCompatibilityError: LocalizedError, Equatable, Sendable {
...
public struct LensSchemaDescriptor: Equatable, Sendable {
    ...
    public func validate(_ version: String) throws {
        guard let candidate = LensSchemaVersion(version) else {
            throw LensSchemaCompatibilityError.malformed(
                document: identifier,
                found: version
            )
        }
        guard let minimum = LensSchemaVersion(minimumReadableVersion),
              let current = LensSchemaVersion(currentVersion),
              candidate >= minimum,
              candidate <= current else {
            throw LensSchemaCompatibilityError.unsupported(
                document: identifier,
                found: version,
...
public enum LensProjectSchema {
    public static let manifest = LensSchemaDescriptor(
        identifier: "manifest.json",
        relativePath: "manifest.json",
        minimumReadableVersion: "0.1",
        currentVersion: LensManifest.currentSchemaVersion
    )
    public static let autoEditPlan = LensSchemaDescriptor(
        identifier: "edit-plan.json",
        relativePath: "edits/edit-plan.json",
        minimumReadableVersion: "0.1",
        currentVersion: AutoEditPlan.currentSchemaVersion
    )
    public static let screenshotEditPlan = LensSchemaDescriptor(
