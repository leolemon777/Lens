...
public enum LensProjectStoreError: LocalizedError, Equatable {
...
public struct LensProjectStore: Sendable {
    ...
    public static var defaultRootDirectory: URL {
        ...
        return pictures.appendingPathComponent("Lens", isDirectory: true)
    }
    ...
    public func saveScreenshot(
        ...
        guard width > 0, height > 0 else {
            throw LensProjectStoreError.invalidDimensions
        }

        let packageURL = uniquePackageURL(createdAt: createdAt, id: id)
        guard !FileManager.default.fileExists(atPath: packageURL.path) else {
            throw LensProjectStoreError.packageAlreadyExists
        }
        ...
        do {
            ...
            let manifest = LensManifest(
                ...
                dimensions: LensDimensions(width: width, height: height),
            ...
            return SavedLens(packageURL: packageURL, rawAssetURL: rawAssetURL, manifest: manifest)
        } catch {
    ...
    public func loadManifest(from packageURL: URL) throws -> LensManifest {
    ...
    private func hydrateModernPointerReplay(
        ...
        let pointers = ((try? LensEventReader.read(
            PointerEvent.self,
            from: pointerURL
    ...
    public func writeAutoEditPlan(
        ...
        guard manifest.kind == .recording else {
            throw LensProjectStoreError.incompatibleLensKind
        }
        ...
        if !manifest.assets.contains(where: { $0.role == .editPlan }) {
            manifest.assets.append(LensAsset(role: .editPlan, relativePath: relativePath))
        }
    ...
    public func loadRecordingSegmentIndex(from packageURL: URL) throws -> RecordingSegmentIndex {
        ...
        try LensProjectSchema.recordingSegments.validate(index.schemaVersion)
        return index
    }

    public func attachOCR(
        _ document: OCRDocument,
        to savedLens: SavedLens
    ) throws -> SavedLens {
        try LensProjectSchema.ocr.validate(document.schemaVersion)
        var manifest = try loadManifest(from: savedLens.packageURL)
        guard manifest.kind == .screenshot else {
            throw LensProjectStoreError.incompatibleLensKind
        }

        let relativePath = "analysis/ocr.json"
        let outputURL = savedLens.packageURL.appendingPathComponent(relativePath)
        try FileManager.default.createDirectory(
            at: outputURL.deletingLastPathComponent(),
            withIntermediateDirectories: true
        )
        ...
        if !manifest.assets.contains(where: { $0.role == .ocr }) {
            manifest.assets.append(LensAsset(role: .ocr, relativePath: relativePath))
        }
        try writeManifest(manifest, to: savedLens.packageURL)
        return SavedLens(
            packageURL: savedLens.packageURL,
            rawAssetURL: savedLens.rawAssetURL,
            manifest: manifest
        )
    }
    ...
    public func attachTranscript(
        ...
        try LensProjectSchema.transcript.validate(document.schemaVersion)
        var manifest = try loadManifest(from: savedLens.packageURL)
        guard manifest.kind == .recording else {
            throw LensProjectStoreError.incompatibleLensKind
        }
        manifest.schemaVersion = LensManifest.currentSchemaVersion
        ...
        if !manifest.assets.contains(where: { $0.role == .transcript }) {
            manifest.assets.append(LensAsset(role: .transcript, relativePath: relativePath))
        }
        try writeManifest(manifest, to: savedLens.packageURL)
        return SavedLens(
            packageURL: savedLens.packageURL,
    ...
    public func attachInsights(
        _ document: LensInsightsDocument,
        to savedLens: SavedLens
    ) throws -> SavedLens {
        try LensProjectSchema.insights.validate(document.schemaVersion)
        var manifest = try loadManifest(from: savedLens.packageURL)
        manifest.schemaVersion = LensManifest.currentSchemaVersion
        ...
        if !manifest.assets.contains(where: { $0.role == .insights }) {
            manifest.assets.append(LensAsset(role: .insights, relativePath: relativePath))
        }
        try writeManifest(manifest, to: savedLens.packageURL)
        return SavedLens(
            packageURL: savedLens.packageURL,
    ...
    public func loadInsights(from packageURL: URL) throws -> LensInsightsDocument {
        let data = try Data(
    ...
    public func writeScreenshotEditPlan(
        ...
        try LensProjectSchema.screenshotEditPlan.validate(plan.schemaVersion)
        var manifest = try loadManifest(from: savedLens.packageURL)
        guard manifest.kind == .screenshot else {
            throw LensProjectStoreError.incompatibleLensKind
        }
        ...
        if !manifest.assets.contains(where: { $0.role == .screenshotEditPlan }) {
            manifest.assets.append(
                LensAsset(role: .screenshotEditPlan, relativePath: relativePath)
            )
        }
        try writeManifest(manifest, to: savedLens.packageURL)
        return SavedLens(
    ...
    public func attachScrollingCapture(
        ...
        try LensProjectSchema.scrollingCapture.validate(plan.schemaVersion)
        var manifest = try loadManifest(from: savedLens.packageURL)
        guard manifest.kind == .screenshot else {
            throw LensProjectStoreError.incompatibleLensKind
        }
        ...
        let rawDirectory = savedLens.packageURL.appendingPathComponent(
            "raw/scrolling",
            isDirectory: true
        )
        let planURL = savedLens.packageURL.appendingPathComponent(
        ...
        let stagingDirectory = savedLens.packageURL.appendingPathComponent(
            "raw/.scrolling-\(stagingID)",
            isDirectory: true
        )
        let stagingPlanURL = savedLens.packageURL.appendingPathComponent(
        ...
        do {
            ...
            manifest.assets.append(contentsOf: plan.frames.map {
                LensAsset(role: .scrollingCaptureFrame, relativePath: $0.relativePath)
            })
            manifest.assets.append(
                LensAsset(
    ...
    public func completeScreenshotEditing(
        ...
        guard FileManager.default.fileExists(atPath: renderedImageURL.path) else {
            throw LensProjectStoreError.missingRenderedScreenshot
        }
        ...
        guard (attributes[.size] as? NSNumber)?.int64Value ?? 0 > 0 else {
            throw LensProjectStoreError.emptyRenderedScreenshot
        }
        ...
        if !manifest.assets.contains(where: { $0.role == .renderedScreenshot }) {
            manifest.assets.append(
                LensAsset(role: .renderedScreenshot, relativePath: relativePath)
            )
        }
        ...
        return SavedLens(packageURL: packageURL, rawAssetURL: rawAsset, manifest: manifest)
    }

    /// Refreshes the disposable persistent index. Invalid packages are skipped; valid interrupted captures remain visible.
    public func libraryEntries() -> [LensLibraryEntry] {
        LensLibraryPersistentIndexStore(
            rootDirectory: rootDirectory,
            indexURL: libraryIndexURL
        ).entries(packageURLs: lensPackageURLs(in: rootDirectory))
    }
    ...
    public func beginRecording(
        ...
        do {
            ...
            var assets = [
                ...
                LensAsset(role: .clickEvents, relativePath: "events/clicks.jsonl"),
                LensAsset(role: .keyboardEvents, relativePath: "events/keyboard.jsonl"),
                LensAsset(role: .windowEvents, relativePath: "events/windows.jsonl"),
                LensAsset(role: .recordingSegments, relativePath: "events/segments.json"),
                LensAsset(role: .editPlan, relativePath: "edits/edit-plan.json")
            ]
            if includesSystemAudio {
                assets.append(
                    LensAsset(role: .systemAudio, relativePath: "raw/screen.mp4")
                )
            }
    ...
    public func finalizeRecording(
        _ session: RecordingLensSession,
        durationSeconds: Double,
        state: LensState = .processing
    ) throws -> SavedLens {
        guard FileManager.default.fileExists(atPath: session.videoURL.path) else {
            throw LensProjectStoreError.missingRawRecording
        }
    ...
    public func writeRecordingHealthReport(
        ...
        manifest.assets.append(
            LensAsset(role: .recordingHealth, relativePath: relativePath)
        )
        try writeManifest(manifest, to: packageURL)
        let rawAssetURL = manifest.assets.first(where: { $0.role == .screenVideo })
            .map { packageURL.appendingPathComponent($0.relativePath) }
    ...
    public func upsertAsset(_ asset: LensAsset, in packageURL: URL) throws {
        var manifest = try loadManifest(from: packageURL)
    ...
    public func appendRecordingSegment(
        ...
        to session: RecordingLensSession
    ) throws {
        ...
        let assets = [
            LensAsset(role: .screenVideoSegment, relativePath: segment.screenRelativePath),
            segment.microphoneRelativePath.map {
                LensAsset(role: .microphoneSegment, relativePath: $0)
            },
            segment.cameraRelativePath.map {
                LensAsset(role: .cameraSegment, relativePath: $0)
            }
        ].compactMap { $0 }
    ...
    public func discardRecordingSegment(
        index segmentIndex: Int,
        from session: RecordingLensSession
    ) throws {
        var index = try loadRecordingSegmentIndex(from: session.packageURL)
        ...
        manifest.assets.removeAll { asset in
            paths.contains(asset.relativePath) && [
                LensAsset.Role.screenVideoSegment,
                .microphoneSegment,
                .cameraSegment
            ].contains(asset.role)
        }
    ...
    public func removeRecordingSegmentMedia(
        ...
        let manifestRole: LensAsset.Role
        switch role {
        case .microphone:
    ...
    public func writeAutoEditPlan(
        for session: RecordingLensSession,
        durationSeconds: Double
    ) throws -> AutoEditPlan {
        ...
        let pointerEvents = try LensEventReader.read(
            PointerEvent.self,
    ...
    public func regeneratedAutomaticCameraKeyframes(
        ...
        let clicks = FileManager.default.fileExists(atPath: clickURL.path)
            ? try LensEventReader.read(ClickEvent.self, from: clickURL)
            : []
        let pointers = FileManager.default.fileExists(atPath: pointerURL.path)
            ? try LensEventReader.read(PointerEvent.self, from: pointerURL)
            : []
    ...
    public func recoverInterruptedRecordings(
        ...
        for url in lensPackageURLs(in: rootDirectory) {
    ...
    public func interruptedRecordingCandidates(
        ...
        lensPackageURLs(in: rootDirectory).compactMap { url in
    ...
    public func completeProcessing(
        ...
        if !manifest.assets.contains(where: { $0.role == .renderedVideo }) {
            manifest.assets.append(LensAsset(role: .renderedVideo, relativePath: relativePath))
        }
        try writeManifest(manifest, to: packageURL)
    ...
    private func lensPackageURLs(in directory: URL) -> [URL] {
        ...
        for child in children {
            ...
            if child.pathExtension == "lens" {
                if values.isDirectory == true { packages.append(child) }
                continue
            }
            guard values.isDirectory == true else { continue }
            packages.append(contentsOf: lensPackageURLs(in: child))
        }
        return packages
    }
    ...
    private func uniquePackageURL(createdAt: Date, id: UUID) -> URL {
        ...
        let basename = "Lens-\(Self.filenameTimestamp.string(from: createdAt))-\(id.uuidString.prefix(8))"
        return dayDirectory.appendingPathComponent("\(basename).lens", isDirectory: true)
    }
