...
public enum ScreenshotAnnotationGeometry {
    ...
    public static func point(
        ...
        switch handle {
        case .topLeft:
            return LensPoint(x: bounds.x, y: bounds.y)
        case .topRight:
            return LensPoint(x: bounds.x + bounds.width, y: bounds.y)
        case .bottomLeft:
            return LensPoint(x: bounds.x, y: bounds.y + bounds.height)
        case .bottomRight:
            return LensPoint(x: bounds.x + bounds.width, y: bounds.y + bounds.height)
    ...
    public static func moved(
        ...
        if let start = annotation.start {
            result.start = LensPoint(x: start.x + deltaX, y: start.y + deltaY)
        }
        if let end = annotation.end {
            result.end = LensPoint(x: end.x + deltaX, y: end.y + deltaY)
        }
        if let points = annotation.points {
            result.points = points.map {
                LensPoint(x: $0.x + deltaX, y: $0.y + deltaY)
            }
        }
        return result
    }
    ...
    public static func resized(
        ...
        let resizedBounds: LensRect

        switch handle {
        case .topLeft:
            let x = min(max(point.x, 0), maxX - minimumExtent)
            let y = min(max(point.y, 0), maxY - minimumExtent)
            resizedBounds = LensRect(x: x, y: y, width: maxX - x, height: maxY - y)
        case .topRight:
            let x = max(min(point.x, 1), minX + minimumExtent)
            let y = min(max(point.y, 0), maxY - minimumExtent)
            resizedBounds = LensRect(x: minX, y: y, width: x - minX, height: maxY - y)
        case .bottomLeft:
            let x = min(max(point.x, 0), maxX - minimumExtent)
            let y = max(min(point.y, 1), minY + minimumExtent)
            resizedBounds = LensRect(x: x, y: minY, width: maxX - x, height: y - minY)
        case .bottomRight:
            let x = max(min(point.x, 1), minX + minimumExtent)
            let y = max(min(point.y, 1), minY + minimumExtent)
            resizedBounds = LensRect(x: minX, y: minY, width: x - minX, height: y - minY)
        case .arrowStart, .arrowEnd:
            return annotation
        }
    ...
    public static func bounds(
        ...
        return bounds(
            from: LensPoint(x: minX, y: minY),
            to: LensPoint(x: maxX, y: maxY),
            minimumExtent: minimumExtent
        )
    }
    ...
    private static func contains(
        ...
        if annotation.kind == .arrow {
            let start = annotation.start
                ?? LensPoint(x: annotation.bounds.x, y: annotation.bounds.y)
    ...
    private static func distance(
        from point: LensPoint,
        toSegmentFrom start: LensPoint,
        to end: LensPoint
    ) -> Double {
    ...
    private static func clamped(_ point: LensPoint) -> LensPoint {
        LensPoint(
    ...
    private static func remapped(
        _ points: [LensPoint],
        from source: LensRect,
        to destination: LensRect,
        minimumExtent: Double
    ) -> [LensPoint] {
