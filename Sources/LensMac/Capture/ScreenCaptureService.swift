...
struct ScreenCaptureService {
    ...
    func availableWindowTargets(
        ...
        let candidates = content.windows.enumerated().compactMap { fallbackIndex, window -> WindowCaptureTarget? in
            ...
            let application = processID.flatMap(NSRunningApplication.init(processIdentifier:))
            let title = window.title?.trimmingCharacters(in: .whitespacesAndNewlines)
            guard Self.isEligibleWindow(
                isOnScreen: window.isOnScreen,
                isRegularApplication: application?.activationPolicy == .regular,
                hasWindowTitle: title?.isEmpty == false,
    ...
    nonisolated static func isEligibleWindow(
        ...
        let isEligibleOffscreenWindow = !isOnScreen
            && isRegularApplication
            && hasWindowTitle
        guard (isOnScreen || isEligibleOffscreenWindow),
              windowLayer == 0,
              frame.width >= 48,
              frame.height >= 32,
