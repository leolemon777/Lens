...
package enum G2SourceInterruptionRunner {
    package static func run(_ configuration: G2SourceInterruptionConfiguration) async -> Int32 {
        ...
        let windowTitle = "Lens G2 Source \(UUID().uuidString)"
        ...
        let store = LensProjectStore(rootDirectory: workRoot)
    ...
    package static func run(_ configuration: G2SystemAudioProbeConfiguration) async -> Int32 {
        ...
        do {
            ...
            let videoURL = FileManager.default.temporaryDirectory.appendingPathComponent(
                "Lens-G2-video-probe-\(UUID().uuidString).mp4"
            )
            ...
            let queue = DispatchQueue(
                label: "app.lens.g2-system-audio-probe",
                qos: .userInteractive
            )
            let stream = SCStream(
                filter: filter,
...
package enum G2RecordingStressRunner {
    package static func run(_ configuration: G2RecordingStressConfiguration) async -> Int32 {
        ...
        let store = LensProjectStore(rootDirectory: temporaryRoot)
        let service = ScreenRecordingService(
            store: store,
            pointerRecorder: PointerEventRecorder()
        )
    ...
    private static func stableFailureCode(_ error: Error) -> String {
        ...
        if error is LensProjectStoreError { return "projectStorageFailed" }
...
package enum G2RecordingRecoveryRunner {
    ...
    private static func stableFailureCode(_ error: Error) -> String {
        ...
        if error is LensProjectStoreError { return "projectRecoveryFailed" }
        return "unexpectedFailure"
    }
...
private final class G2EnduranceToneGenerator {
    ...
    func start() throws {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "Lens-G2-tone-\(UUID().uuidString).caf"
        )
