            ...
            if configuration.persistsDerivedCopy {
                workingProjectURL = configuration.projectURL
                    ...
                    .appendingPathComponent(
                        ".g3-roundtrip-\(UUID().uuidString).lens",
                        isDirectory: true
                    )
            ...
            if configuration.persistsDerivedCopy {
                ...
                if configuration.addsVideoAnnotation {
                    ...
                    guard editorModel.commitVideoAnnotationDraft(
                        start: LensPoint(x: 0.18, y: 0.18),
                        end: LensPoint(x: 0.82, y: 0.82),
                ...
                var savedLens = try store.writeAutoEditPlan(
                    plan,
                    to: workingProjectURL
                )
                savedLens = try store.attachTranscript(
                    authoredTranscript,
                    to: savedLens
                )
                let insights = LocalLensOrganizer.organize(
                    manifest: savedLens.manifest,
                    transcript: authoredTranscript
                )
                savedLens = try store.attachInsights(insights, to: savedLens)
                ...
                let requiredRoles: Set<LensAsset.Role> = [
