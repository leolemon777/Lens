...
final class GlobalHotKeyManager {
    ...
    @discardableResult
    func resume() -> HotKeyRegistrationReport {
        guard started, isSuspended else { return .notStarted }
        isSuspended = false
        return installBindings()
    }
    ...
    func handleNativeHotKey(identifier: UInt32) {
        guard started, !isSuspended else { return }
    ...
    private func handleMonitoredInput(_ input: HotKeyInput, generation: UInt64) {
        guard started, !isSuspended, generation == monitorGeneration else { return }
        if let intent = stateMachine.handle(input) {
            handler(intent)
        }
    }

    private func installBindings() -> HotKeyRegistrationReport {
