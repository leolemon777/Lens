...
struct ScreenTraceCrashSummary: Codable, Equatable, Sendable {
    let incidentID: UUID
    let timestamp: Date
...
@MainActor
final class ScreenTraceCrashReportScanner {
    ...
    init(
        directory: URL = ScreenTraceCrashReportScanner.defaultDirectory,
    ...
    func recentReports(limit: Int = 5) -> [ScreenTraceCrashSummary] {
        guard limit > 0,
        ...
        return urls.compactMap { url -> (Date, URL)? in
            guard url.lastPathComponent.hasPrefix("ScreenTrace-"),
    ...
    private func parse(_ url: URL) -> ScreenTraceCrashSummary? {
        ...
        guard let header = jsonObject(Data(headerData)),
              ...
              header["bundleID"] as? String == "app.screentrace.mac",
              header["app_name"] as? String == "ScreenTrace",
        ...
        return ScreenTraceCrashSummary(
