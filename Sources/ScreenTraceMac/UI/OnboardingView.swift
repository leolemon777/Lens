...
struct OnboardingView: View {
    ...
    private var welcomeStep: some View {
        VStack(alignment: .leading, spacing: 16) {
            Text("屏迹把截图和录屏放在同一个本地工作台里。所有素材、事件轨和编辑计划都留在你自己的磁盘上。")
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
    ...
    private var essentialsStep: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("没有这两项，截图和录屏无法开始。屏迹只在你主动触发捕捉时读取屏幕。")
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            ...
            if model.suggestsRelaunch {
                Label(
                    "屏幕录制权限对已经在运行的应用不会立即生效。如果你刚刚在系统设置里允许了屏迹，需要退出后重新打开一次。",
    ...
    private var smartStep: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("屏迹的自动运镜、光标重绘和点击波纹都是从真实的指针与点击事件规划出来的。没有这项权限，录屏仍然能录，但成片就是一段普通录屏。")
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
