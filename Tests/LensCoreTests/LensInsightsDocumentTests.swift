...
final class LensInsightsDocumentTests: XCTestCase {
    ...
    func testDocumentNormalizesOpenSchemaAndRoundTrips() throws {
        let document = LensInsightsDocument(
            engine: " local-test ",
        ...
        XCTAssertEqual(decoded.schemaVersion, LensInsightsDocument.currentSchemaVersion)
    ...
    func testInsightsPersistBecomeSearchableAndInvalidateDisposableIndex() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("LensInsightsTests-\(UUID().uuidString)", isDirectory: true)
        ...
        let first = LensInsightsDocument(
            engine: "test",
            generatedAt: Date(timeIntervalSince1970: 1),
            suggestedTitle: "Launch planning",
            summary: "Prepare the Lens public release.",
        ...
        let customized = first.replacingCustomization(LensInsightsCustomization(
        ...
        let second = LensInsightsDocument(
            engine: "test",
