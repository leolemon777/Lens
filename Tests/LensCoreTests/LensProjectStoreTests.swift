...
final class LensProjectStoreTests: XCTestCase {
    func testScreenshotRoundTripCreatesOpenProjectPackage() throws {
        ...
        XCTAssertEqual(manifest.schemaVersion, LensManifest.currentSchemaVersion)
        XCTAssertEqual(manifest.id, id)
        XCTAssertEqual(manifest.kind, .screenshot)
        XCTAssertEqual(manifest.dimensions, LensDimensions(width: 1440, height: 900))
    ...
    func testInvalidDimensionsDoNotCreatePackage() throws {
        ...
        XCTAssertThrowsError(try store.saveScreenshot(
            ...
            XCTAssertEqual(error as? LensProjectStoreError, .invalidDimensions)
        }
    ...
    func testOCRRoundTripAddsAnalysisAssetWithoutChangingRawScreenshot() throws {
        ...
        let document = OCRDocument(
            ...
            blocks: [
                OCRTextBlock(
                    text: "Lens",
                    confidence: 0.98,
                    normalizedBounds: LensRect(x: 0.1, y: 0.2, width: 0.5, height: 0.1)
                )
            ]
        )
        ...
        XCTAssertEqual(updatedAgain.manifest.assets.filter { $0.role == .ocr }, [
            LensAsset(role: .ocr, relativePath: "analysis/ocr.json")
        ])
        XCTAssertEqual(document.fullText, "Lens")
    }
    ...
    func testOCRCannotBeAttachedToRecordingProject() throws {
        ...
        XCTAssertThrowsError(try store.attachOCR(document, to: recording)) { error in
            XCTAssertEqual(error as? LensProjectStoreError, .incompatibleLensKind)
        }
    }
        ...
        let plan = ScreenshotEditPlan(
            ...
            annotations: [
                ScreenshotAnnotation(
                    kind: .rectangle,
                    bounds: LensRect(x: 0.1, y: 0.1, width: 0.4, height: 0.3)
                )
            ],
    ...
    func testScreenshotEditPlanRejectsMismatchedSourceDimensions() throws {
        ...
        let plan = ScreenshotEditPlan(
            sourceDimensions: LensDimensions(width: 801, height: 500)
        )
    ...
    func testRecordingManifestPersistsWindowCaptureContext() throws {
        ...
        let session = try store.beginRecording(
            width: 1_800,
            height: 1_200,
            captureSource: LensCaptureMetadata(recordingSource: source, framesPerSecond: 30)
        )
    ...
    func testLegacyPointOneManifestDecodesWithoutCaptureMetadata() throws {
        ...
        let manifest = try decoder.decode(LensManifest.self, from: Data(json.utf8))

        XCTAssertEqual(manifest.schemaVersion, "0.1")
        XCTAssertNil(manifest.captureSource)
        XCTAssertEqual(manifest.dimensions, LensDimensions(width: 1_280, height: 720))
    }
    ...
    func testRecordingCannotFinalizeWithoutNonemptyRawVideo() throws {
        ...
        XCTAssertThrowsError(try store.finalizeRecording(session, durationSeconds: 1)) { error in
            XCTAssertEqual(error as? LensProjectStoreError, .missingRawRecording)
        }
        FileManager.default.createFile(atPath: session.videoURL.path, contents: nil)
        XCTAssertThrowsError(try store.finalizeRecording(session, durationSeconds: 1)) { error in
            XCTAssertEqual(error as? LensProjectStoreError, .emptyRawRecording)
        }
    }
    ...
    func testAutoEditPlanUsesRecordedClickTrack() async throws {
        ...
        try await writer.append(ClickEvent(
            ...
            button: .left,
            phase: .down,
            location: LensPoint(x: 900, y: 500),
            normalizedLocation: LensPoint(x: 0.47, y: 0.46),
            displayID: 1,
            clickCount: 1,
        ...
        try await pointerWriter.append(PointerEvent(
            time: 0.5,
            kind: .moved,
            location: LensPoint(x: 1_000, y: 700),
            normalizedLocation: LensPoint(x: 0.8, y: 0.2),
            displayID: 1,
            cursorShape: .arrow
        ))
        ...
        XCTAssertEqual(
            ...
            [AutoEditPlan.CursorKeyframe(
                time: 0.5,
                position: LensPoint(x: 0.8, y: 0.2),
                kind: .moved
            )]
        )
        XCTAssertEqual(
    ...
    func testVersionOnePlanMigratesToDistanceAwareCameraWithoutRewritingPackage() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("LensCameraMigrationTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let store = LensProjectStore(rootDirectory: root)
        ...
        for click in [
            ClickEvent(
                ...
                button: .left,
                phase: .down,
                location: LensPoint(x: 640, y: 720),
                normalizedLocation: LensPoint(x: 0.25, y: 0.5),
                displayID: 1,
                clickCount: 1
            ),
            ClickEvent(
                ...
                button: .left,
                phase: .down,
                location: LensPoint(x: 1_920, y: 720),
                normalizedLocation: LensPoint(x: 0.75, y: 0.5),
                displayID: 1,
                clickCount: 1
            )
        ] {
        ...
        legacyPlan.camera.keyframes = [
            ...
            AutoEditPlan.CameraKeyframe(
                time: 1.82,
                scale: 1.6,
                center: LensPoint(x: 0.3125, y: 0.5),
            ...
            AutoEditPlan.CameraKeyframe(
                time: 3.82,
                scale: 1.6,
                center: LensPoint(x: 0.6875, y: 0.5),
                easing: "ease-in-out-smootherstep",
                reason: .clickFocus
            )
        ]
    ...
    func testAutoEditPlanDropsEventsOutsideFinalMediaDuration() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("LensEventBoundsTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let store = LensProjectStore(rootDirectory: root)
        ...
        for time in [1.0, 8.0] {
            try await clickWriter.append(ClickEvent(
                ...
                button: .left,
                phase: .down,
                location: LensPoint(x: 640, y: 360),
        ...
        for time in [0.5, 9.0] {
            try await pointerWriter.append(PointerEvent(
                time: time,
                kind: .moved,
                location: LensPoint(x: 400, y: 300),
                normalizedLocation: LensPoint(x: 0.3, y: 0.4),
                displayID: 1
            ))
        }
        try await pointerWriter.close()
    ...
    func testAutomaticCameraCanRegenerateFromEventsWithIndependentControls() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("LensRegenerateTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let store = LensProjectStore(rootDirectory: root)
        let session = try store.beginRecording(width: 1_280, height: 720)
        let writer = try JSONLinesWriter<ClickEvent>(url: session.clickEventsURL)
        ...
        plan.videoAnnotations = [
            VideoAnnotation(
                annotation: ScreenshotAnnotation(
                    kind: .arrow,
                    bounds: LensRect(x: 0.1, y: 0.2, width: 0.3, height: 0.4),
                    start: LensPoint(x: 0.1, y: 0.2),
                    end: LensPoint(x: 0.4, y: 0.6)
                ),
    ...
    func testScrollingCaptureArchivesEverySourceFrameAndAssemblyPlan() throws {
        ...
        let plan = ScrollingCapturePlan(
            displayID: 7,
            sourceRect: LensRect(x: 100, y: 80, width: 400, height: 300),
            viewportDimensions: LensDimensions(width: 800, height: 600),
            outputDimensions: LensDimensions(width: 800, height: 1_070),
            frames: frames
        )
        ...
        XCTAssertThrowsError(try store.attachScrollingCapture(
            ...
            XCTAssertEqual(error as? LensProjectStoreError, .packageAlreadyExists)
        }
        XCTAssertEqual(
            try Data(contentsOf: screenshot.packageURL.appendingPathComponent(frames[0].relativePath)),
            Data([9, 8])
