...
final class MicrophoneTrackRecorderTests: XCTestCase {
    func testWriterPersistsSyntheticNarrationAsIndependentCAFTrack() throws {
        let outputURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("LensMicrophoneTests-\(UUID().uuidString).caf")
    ...
    func testTapHandlerCanRunOnAudioQueue() throws {
        let outputURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("LensMicrophoneTapTests-\(UUID().uuidString).caf")
        defer { try? FileManager.default.removeItem(at: outputURL) }
        let format = try XCTUnwrap(
            AVAudioFormat(standardFormatWithSampleRate: 48_000, channels: 1)
        )
        ...
        DispatchQueue(label: "app.lens.tests.microphone-tap").sync {
            invocation.run()
        }
