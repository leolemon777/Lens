...
final class ScreenshotAnnotationRendererTests: XCTestCase {
    func testVectorAnnotationsRenderVisibleRectangleArrowAndText() throws {
        ...
        let dimensions = LensDimensions(width: source.width, height: source.height)
        let plan = ScreenshotEditPlan(
            sourceDimensions: dimensions,
            annotations: [
                ScreenshotAnnotation(
                    kind: .rectangle,
                    bounds: LensRect(x: 0.08, y: 0.08, width: 0.34, height: 0.32),
                    style: ScreenshotAnnotationStyle(lineWidth: 0.018, color: .red)
                ),
                ScreenshotAnnotation(
                    kind: .arrow,
                    bounds: LensRect(x: 0.12, y: 0.75, width: 0.72, height: -0.48),
                    start: LensPoint(x: 0.12, y: 0.75),
                    end: LensPoint(x: 0.84, y: 0.27),
                    style: ScreenshotAnnotationStyle(lineWidth: 0.014, color: .blue)
                ),
                ScreenshotAnnotation(
                    kind: .text,
                    bounds: LensRect(x: 0.50, y: 0.72, width: 0.4, height: 0.1),
                    text: "Lens",
    ...
    func testGradientArrowRendersBothEndpointColors() throws {
        ...
        let plan = ScreenshotEditPlan(
            ...
            annotations: [ScreenshotAnnotation(
                kind: .arrow,
                bounds: LensRect(x: 0.1, y: 0.2, width: 0.8, height: 0.55),
                start: LensPoint(x: 0.1, y: 0.2),
                end: LensPoint(x: 0.9, y: 0.75),
    ...
    func testBlurAndPixelateChangeTargetRegionsButPreserveOutsidePixels() throws {
        ...
        let plan = ScreenshotEditPlan(
            ...
            annotations: [
                ScreenshotAnnotation(
                    kind: .blur,
                    bounds: LensRect(x: 0.12, y: 0.25, width: 0.28, height: 0.5),
                    style: ScreenshotAnnotationStyle(intensity: 0.05)
                ),
                ScreenshotAnnotation(
                    kind: .pixelate,
                    bounds: LensRect(x: 0.60, y: 0.25, width: 0.28, height: 0.5),
        ...
        let leftDifference = meanDifference(
            ...
            normalizedRect: LensRect(x: 0.14, y: 0.30, width: 0.24, height: 0.40)
        )
        let rightDifference = meanDifference(
            ...
            width: source.width,
            height: source.height,
            normalizedRect: LensRect(x: 0.62, y: 0.30, width: 0.24, height: 0.40)
        )
        let outsideDifference = meanDifference(
            ...
            width: source.width,
            height: source.height,
            normalizedRect: LensRect(x: 0.43, y: 0.05, width: 0.12, height: 0.14)
        )
    ...
    func testFreehandHighlightAndStepRenderAsDistinctEditableObjects() throws {
        ...
        let plan = ScreenshotEditPlan(
            ...
            annotations: [
                ScreenshotAnnotation(
                    kind: .freehand,
                    bounds: LensRect(x: 0.08, y: 0.12, width: 0.72, height: 0.26),
                    points: [
                        LensPoint(x: 0.08, y: 0.28),
                        LensPoint(x: 0.28, y: 0.12),
                        LensPoint(x: 0.52, y: 0.38),
                        LensPoint(x: 0.80, y: 0.18)
                    ],
                    style: ScreenshotAnnotationStyle(lineWidth: 0.018, color: .blue)
                ),
                ScreenshotAnnotation(
                    kind: .highlight,
                    bounds: LensRect(x: 0.12, y: 0.52, width: 0.56, height: 0.14),
                    style: ScreenshotAnnotationStyle(
                        lineWidth: 0,
                        color: .yellow,
                        fillColor: LensColor(red: 1, green: 0.8, blue: 0, alpha: 0.34)
                    )
                ),
                ScreenshotAnnotation(
                    kind: .step,
                    bounds: LensRect(x: 0.78, y: 0.62, width: 0.12, height: 0.18),
                    text: "3",
    ...
    func testMismatchedDimensionsAreRejected() throws {
        ...
        let plan = ScreenshotEditPlan(sourceDimensions: LensDimensions(width: 101, height: 80))
    ...
    func testCanvasBackgroundExpandsOutputAndPreservesSourcePixels() throws {
        ...
        let plan = ScreenshotEditPlan(
            sourceDimensions: LensDimensions(width: 400, height: 200),
    ...
    func testCanvasShadowDoesNotFillTransparentSourceGaps() throws {
        ...
        let plan = ScreenshotEditPlan(
            sourceDimensions: LensDimensions(width: 100, height: 100),
            canvasStyle: ScreenshotCanvasStyle(
                backgroundKind: .solid,
                primaryColor: .red,
                secondaryColor: .red,
