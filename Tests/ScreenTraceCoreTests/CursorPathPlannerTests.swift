...
final class CursorPathPlannerTests: XCTestCase {
        ...
        XCTAssertEqual(
            ...
            [
                AutoEditPlan.CursorKeyframe(
                    time: 0.1,
                    position: TracePoint(x: 0.2, y: 0.8),
                    kind: .moved
                ),
                AutoEditPlan.CursorKeyframe(
                    time: 0.2,
                    position: TracePoint(x: 0.9, y: 0.1),
                    kind: .moved
                )
            ]
        )
    ...
    func testRawAndSmoothedPlansPreserveDragState() {
        let events = [
            PointerEvent(
                ...
                normalizedLocation: TracePoint(x: 0.1, y: 0.1),
                displayID: 1
            ),
            PointerEvent(
                time: 0.2,
                kind: .dragged,
                location: TracePoint(x: 300, y: 200),
                normalizedLocation: TracePoint(x: 0.3, y: 0.2),
                displayID: 1
            )
        ]
    ...
    func testScrollSamplesDoNotBecomeSyntheticCursorMovement() {
        ...
        let scroll = PointerEvent(
            time: 0.2,
            kind: .scroll,
            location: TracePoint(x: 200, y: 800),
            normalizedLocation: TracePoint(x: 0.2, y: 0.8),
            displayID: 1,
            scrollDelta: TracePoint(x: 0, y: -12)
        )
    ...
    func testShapePlanCombinesPointerAndClickSamplesAndDeduplicatesChanges() {
        ...
        let handClick = ClickEvent(
            ...
            button: .left,
            phase: .down,
            location: TracePoint(x: 400, y: 200),
            clickCount: 1,
