...
final class EffectTimelineTests: XCTestCase {
    ...
    func testCursorKindUsesSeekSafeDragStateAcrossLargeTracks() {
        let keyframes = (0..<20_000).map { index in
            AutoEditPlan.CursorKeyframe(
                ...
                position: TracePoint(x: Double(index % 100) / 100, y: 0.5),
    ...
    func testCursorSmoothingWindowUsesMillisecondsAndZeroRestoresLinearPath() throws {
        let keyframes = [
            AutoEditPlan.CursorKeyframe(time: 0, position: TracePoint(x: 0, y: 0)),
            AutoEditPlan.CursorKeyframe(time: 0.1, position: TracePoint(x: 0, y: 0)),
            AutoEditPlan.CursorKeyframe(time: 0.2, position: TracePoint(x: 1, y: 1)),
            AutoEditPlan.CursorKeyframe(time: 0.3, position: TracePoint(x: 1, y: 0))
        ]
