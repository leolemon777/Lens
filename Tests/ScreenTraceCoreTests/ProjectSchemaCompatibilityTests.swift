...
final class ProjectSchemaCompatibilityTests: XCTestCase {
    func testRegistryMatchesEveryCurrentPortableDocumentVersion() {
        XCTAssertEqual(TraceProjectSchema.manifest.currentVersion, TraceManifest.currentSchemaVersion)
        XCTAssertEqual(TraceProjectSchema.autoEditPlan.currentVersion, AutoEditPlan.currentSchemaVersion)
        XCTAssertEqual(
            TraceProjectSchema.screenshotEditPlan.currentVersion,
            ScreenshotEditPlan.currentSchemaVersion
        )
        XCTAssertEqual(
            TraceProjectSchema.recordingSegments.currentVersion,
            RecordingSegmentIndex.currentSchemaVersion
        )
        XCTAssertEqual(
            TraceProjectSchema.scrollingCapture.currentVersion,
            ScrollingCapturePlan.currentSchemaVersion
        )
        XCTAssertEqual(TraceProjectSchema.ocr.currentVersion, OCRDocument.currentSchemaVersion)
        XCTAssertEqual(
            TraceProjectSchema.transcript.currentVersion,
            TranscriptDocument.currentSchemaVersion
        )
        XCTAssertEqual(
            TraceProjectSchema.insights.currentVersion,
            TraceInsightsDocument.currentSchemaVersion
        )
        XCTAssertEqual(Set(TraceProjectSchema.portableDocuments.map(\.relativePath)).count, 8)
    }
    ...
    func testFutureAndMalformedSchemasAreRejectedInsteadOfSilentlyOpened() throws {
        ...
        XCTAssertThrowsError(try TraceProjectSchema.manifest.validate("future")) { error in
            XCTAssertEqual(
                error as? TraceSchemaCompatibilityError,
    ...
    func testWriterRejectsUnsupportedSchemaBeforeMutatingProject() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("ScreenTraceSchemaWriteTests-\(UUID().uuidString)", isDirectory: true)
        ...
        let unsupported = ScreenshotEditPlan(
            schemaVersion: "9.9",
            sourceDimensions: TraceDimensions(width: 640, height: 360)
        )
    ...
    private func workingCopyOfFixture(named name: String) throws -> URL {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("ScreenTraceSchemaTests-\(UUID().uuidString)", isDirectory: true)
        let destination = root.appendingPathComponent("\(name).screentrace", isDirectory: true)
