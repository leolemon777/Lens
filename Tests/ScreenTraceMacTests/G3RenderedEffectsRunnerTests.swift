...
final class G3RenderedEffectsRunnerTests: XCTestCase {
    func testConfigurationResolvesProjectReportOutputAndPreset() throws {
        let root = URL(fileURLWithPath: "/tmp/ScreenTrace G3", isDirectory: true)
        let configuration = try XCTUnwrap(G3RenderedEffectsConfiguration(
            arguments: [
                "ScreenTrace",
                "--g3-rendered-effects",
                "--project", "Input.screentrace",
        ...
        XCTAssertEqual(
            ...
            "/tmp/ScreenTrace G3/Input.screentrace"
        )
        XCTAssertEqual(
            configuration.reportURL.path,
            "/tmp/ScreenTrace G3/Reports/source.json"
        )
        XCTAssertEqual(
            configuration.outputURL?.path,
            "/tmp/ScreenTrace G3/Outputs/source.mp4"
        )
    ...
    func testTranscriptionConfigurationResolvesControlledEvidenceInputs() throws {
        let root = URL(fileURLWithPath: "/tmp/ScreenTrace Speech", isDirectory: true)
        let configuration = try XCTUnwrap(G3TranscriptionConfiguration(
            arguments: [
                "ScreenTrace", "--g3-transcription",
                "--audio", "speech.aiff",
                "--report", "report.json",
                "--locale", "zh-CN",
        ...
        XCTAssertEqual(configuration.audioURL.path, "/tmp/ScreenTrace Speech/speech.aiff")
        XCTAssertEqual(configuration.reportURL.path, "/tmp/ScreenTrace Speech/report.json")
