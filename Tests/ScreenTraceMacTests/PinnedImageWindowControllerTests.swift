...
final class PinnedImageWindowControllerTests: XCTestCase {
    ...
    func testHidingAllPinsKeepsThemUntilShownOrClosed() {
        ...
        let first = makeTraceAndImage()
        let second = makeTraceAndImage()
        controller.pin(trace: first.trace, image: first.image)
        controller.pin(trace: second.trace, image: second.image)
        ...
        controller.hideAll()
        controller.pin(trace: makeTraceAndImage().trace, image: makeTraceAndImage().image)
    ...
    private func makeTraceAndImage() -> (trace: SavedTrace, image: NSImage) {
