...
@MainActor
final class TraceLibraryRecoveryNoticeTests: XCTestCase {
    private func makeLibrary(
        findings: [RecordingRecoveryAssessment.Finding],
        presented: Double,
        available: Double
    ) throws -> (Data, TraceLibraryEntry) {
        ...
        let package = root.appendingPathComponent("recording.screentrace")
        ...
        if let directory = ProcessInfo.processInfo.environment[
            "SCREENTRACE_RECOVERY_SNAPSHOT_DIRECTORY"
        ] {
            try? FileManager.default.createDirectory(
