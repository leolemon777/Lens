...
5. **不要动 `Sources/LensCore/` 的 import。** 该目录只允许 `import Foundation`（`CaptureGeometry.swift` 和 `CoreWorkerRuntime.swift` 是既有例外）。任何需要 AppKit / AVFoundation / NaturalLanguage 的代码只能放在 `Sources/LensMac/`。
...
`Sources/LensMac/Capture/VideoExportProfile.swift` 里 `.compact`（用户界面上的「轻量」）使用了 `AVAssetExportPresetMediumQuality`。
...
影响两条导出路径：
- `Sources/LensMac/Capture/AutoPreviewRenderer.swift:318`
- `Sources/LensMac/Capture/AudioMixdownRenderer.swift:272`

两处都读同一个 `exportProfile.assetExportPresetName`，所以**只需改 `VideoExportProfile.swift` 一处**，这两条路径同时被修好。不要去改那两个渲染器。

## 1.2 第一步：先写测试（此时必须失败）

编辑 `Tests/LensMacTests/VideoExportProfileTests.swift`。
...
编辑 `Sources/LensMac/Capture/VideoExportProfile.swift`。
...
`Sources/LensCore/TranscriptChunking.swift:43` 的默认重叠是 1 秒：
...
而 `Sources/LensMac/Capture/LocalSpeechTranscriptionService.swift:105` 用的就是默认值：
```swift
...
编辑 `Tests/LensCoreTests/TranscriptChunkingTests.swift`，在类的最后一个方法后新增：
...
编辑 `Sources/LensCore/TranscriptChunking.swift`。
...
编辑 `Sources/LensCore/TranscriptChunking.swift` 的 `TranscriptChunkMerger.merge`。
...
`Sources/LensMac/Capture/VerticalScrollingCaptureAssembler.swift:117`：
...
编辑 `Tests/LensMacTests/VerticalScrollingCaptureAssemblerTests.swift`，新增：
...
编辑 `Sources/LensMac/Capture/VerticalScrollingCaptureAssembler.swift`。
...
`Sources/LensCore/LocalLensOrganizer.swift:101` 的 `extractiveSummary` 名字叫「抽取式摘要」，实际是**顺序取前两句**，没有任何排序。而同文件 `salientSentences`（第 117 行）**已经实现了完整的词频 + 位置加权排序**。结果是要点比摘要聪明——摘要只是开场白。

## 4.2 改动

编辑 `Sources/LensCore/LocalLensOrganizer.swift`。
...
**允许更新的已有断言：** `Tests/LensCoreTests/LocalLensOrganizerTests.swift:88`
...
XCTAssertTrue(
    insights.summary.contains("Lens") || insights.summary.contains("今天"),
    "摘要应当来自转写正文"
)
```
...
`Sources/LensCore/LocalLensOrganizer.swift` 的 `sensitiveMatches` 里，同一个函数用了两套严格度：
    ...
    func testEighteenDigitNumberWithoutAValidChecksumIsNotReported() {
        let ocr = OCRDocument(
            ...
            blocks: [OCRTextBlock(
                ...
                confidence: 0.9,
                boundingBox: LensRect(x: 0, y: 0, width: 1, height: 1)
            )]
        )
...
- `LocalLensOrganizerTests` 里断言 `sensitiveFindings.map(\.kind) == [.phoneNumber]` 的那个（用的是 `13800138000`，符合 `1` + `3` 的大陆号段，会继续命中）
...
`Sources/LensCore/AutoCameraPlanner.swift:368`，在 `pointerActivityBase` 的循环里：
...
编辑 `Sources/LensCore/AutoCameraPlanner.swift`。
...
`Sources/LensCore/LocalLensOrganizer.swift` 的 `hanTokens`（第 527 行）对每段汉字滑窗生成**所有相邻二字组合**，再靠 24 个词的 `hanStopWords` 过滤。"我们来看这个配置文件" 会产出 `来看`、`看这`、`这个`、`个配`、`配置`…，出现 ≥2 次就成为标签——「个配」这种碎片会真的进标签栏。
...
新建文件 `Sources/LensCore/LensTokenizing.swift`：
...
`LocalLensOrganizer.organize` 的签名改为：
```swift
...
新建 `Sources/LensMac/Support/NaturalLanguageTokenizer.swift`：
...
`Sources/LensMac/AppDelegate.swift` 的 `beginOrganization` 里，找到：
```swift
...
- ❌ **绝对不要**在 `Sources/LensCore/` 里 `import NaturalLanguage`
