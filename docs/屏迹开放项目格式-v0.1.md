...
状态：Public Beta 前冻结候选。本文记录当前 App 实际读写的 `.screentrace` 包，不承诺尚未实现的云服务或 Windows 平台细节。

## 设计约束

- `.screentrace` 是普通目录包；用户原始截图、录屏、麦克风和摄像头素材位于 `raw/`，自动处理不得覆盖它们。
...
| 自动编辑计划 | `edits/edit-plan.json` | 0.1 | 0.8 | 运镜、光标、音频、画中画、时间线、字幕、标注和导出意图 |
...
源码中的 `TraceProjectSchema.portableDocuments` 是这张表的可执行权威来源，测试会校验它与各模型的 `currentSchemaVersion` 一致。`.index/library-v1.json` 是可删除重建的本机缓存，不属于开放项目格式。
...
仓库在 `Tests/ScreenTraceCoreTests/Fixtures/` 保留两个不含真实媒体的最小项目：
...
`ProjectSchemaCompatibilityTests` 将样本复制到临时 `.screentrace` 包后，通过真实 `TraceProjectStore` 逐项读取，比较读取前后的全部文件字节，并验证未来/畸形版本被拒绝且不会进入屏迹库。样本不包含图片、音视频或用户数据。
